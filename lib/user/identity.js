define([
  ...
  function verifyUser (user, type, hash, callback) {
  ...
  return {
    ...
    verify: function (user, type, hash, callback) {
        if (typeof hash === 'function') {
          ...
          this.getIdentity(callback, function(error, identity) {
            if (error) {
              return callback(null, error);
            }

            verifyUser(identity.id, type, hash, callback);
          });
        ...
        verifyUser(user, type, hash, callback);
    },
