define(['../core/transport/main'], function(transport) {
  ...
  return {
    ...
    login: function(role, username, password, callback) {
      transport.connection.post('/user/login', {username: username, password: password, role: role}, function(error, result) {
        if (error) {
    ...
    getUsername : function(userId, callback) {
      transport.connection.get('/user/username/' + userId, function(error, response) {
        if (error) {
          return callback(error);
        }
    ...
    getIdentity: function(role, callback, force) {
      ...
      transport.connection.get(requestUri, function(error, result) {
        if (error) {
          return callback(error);
        }
