define([
  ...
  return {
     ...
     */
    updateUser: function (user, properties, callback) {
      if (typeof user === 'object') {
        user = user.user ? (typeof user.user === 'object' ? user.user.id : user.user) : user.id;
      }

      transport.connection.put('/user/' + user, properties, function (response) {
        if (response.error) {
          return callback(response);
        }
