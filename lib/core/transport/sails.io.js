define(['./socket.io', '../utilities/cookie', '../config'], function (io, cookie, config) {

  var apiVersion = typeof config.version === 'number' ? config.version : ('v' + config.version.replace(/^v/, ''));
  ...
  function SailsIOClient (io) {
    ...
    function TmpSocket () {
      ...
      this.on = function (evName, fn) {
        if (!boundEvents[evName]) {
          boundEvents[evName] = [fn];
        } else {
          boundEvents[evName].push(fn);
        }
        return this;
      };
    ...
    io.connect = function (url, opts) {
      ...
      if (typeof opts.query !== 'string') {
        opts.query = SDK_INFO.versionString;
      } else {
        opts.query += '&' + SDK_INFO.versionString;
      }

      opts.query += '&__api_version=' + apiVersion;
    ...
    setTimeout(function () {
      ...
      var gotCookie = cookie.read('io_gotCookie');

      if (safari) {
        if (!gotCookie) {
          cookie.write('io_gotCookie', 1);
          window.top.location = io.sails.url + '/connect/safari-getcookie';
        } else {
          cookie.destroy('io_gotCookie')
        }
      }

      // var port = global.location.port || ('https:' == global.location.protocol ? 443 : 80);
      // this.options.host !== global.location.hostname || this.options.port != port;
      if (io.sails.useCORSRouteToGetCookie && isXOrigin && !safari) {
        // Figure out the x-origin CORS route
        // (Sails provides a default)

        var xOriginCookieRoute = '/connect/getcookie?callback=uponHavingACookieDoThis';
        ...
        window.uponHavingACookieDoThis = function () {
      ...
      function goAheadAndActuallyConnect () {
        ...
        // IE8 hack, doesnt support delete.
        window.uponHavingACookieDoThis = null;
        try {
          delete window.uponHavingACookieDoThis;
        } catch (e) {
        }
        ...
        function onConnectFailure () {
          if (actualSocket.socket.options.port === config.fallbackPort) {
            throw 'Connecting failed. Tried port ' + config.fallbackPort + '. Fallback was same port, so no retries done.';
          }

          if (triedFallback) {
            throw 'Connecting failed. Tried supplied port and fallback ('+config.fallbackPort+').';
          }

          if (actualSocket.socket.origTransports) {
            actualSocket.socket.transports = actualSocket.socket.origTransports;
          }

          actualSocket.socket.options.port = config.fallbackPort;
          actualSocket.socket.connect();

          triedFallback = true;
        }

        actualSocket.on('connect', function () {
          actualSocket.removeListener('error',          onConnectFailure);
          actualSocket.removeListener('connect_failed', onConnectFailure);
        });

        actualSocket.on('error',          onConnectFailure);
        actualSocket.on('connect_failed', onConnectFailure);
        ...
        io.socket.on('connect', function socketConnected () {
          ...
          if (!io.socket.$events.error) {
            io.socket.on('error', function failedToConnect (err) {
              ...
              consolog('Failed to connect socket (probably due to failed authorization on server)');
            });
          }
        });
      }
