define(['../config'], function (config) {
  var endpoint = config.endpoint,
      apiVersion = typeof config.version === 'number' ? config.version : ('v' + config.version.replace(/^v/, '')),
      jsonpTimeout = config.jsonpTimeout || 1500,
      isXOrigin = endpoint && endpoint.replace(/^[a-z]+:\/\//i, '').search(location.hostname) !== 0;
  ...
  /**
   * Send a request to the backend via a XHR
   *
   * @param {string} url
   * @param {{}|function} data
  ...
  /**
   * Send a request to the backend via JSONP
   *
   * @param {string} url
   * @param {{}|function} data
   * @param {Function} callback
  ...
  function jsonpRequest (url, data, callback) {
    var callbackName = '__islcb_' + (new Date()).getTime() + '_' + Math.floor((Math.random() * 100) + 1),
        script = document.createElement('script'),
        timedResponseHandler,
        queryString;
    ...
    window[callbackName] = function (response) {
      ...
      if (timedResponseHandler) {
        clearTimeout(timedResponseHandler);
      }
    ...
    timedResponseHandler = setTimeout(function () {
      window[callbackName]({
  ...
  function request (url, data, callback, method) {
    ...
    data.__api_version = apiVersion;

    if (isXOrigin) {
      return jsonpRequest(url, data, callback, method);
    }
